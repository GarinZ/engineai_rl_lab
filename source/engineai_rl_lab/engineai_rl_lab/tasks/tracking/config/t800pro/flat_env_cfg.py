...
from engineai_rl_lab.tasks.tracking.robots.t800pro import T800pro_ACTION_SCALE, T800pro_CYLINDER_CFG
from engineai_rl_lab.tasks.tracking.config.t800pro.agents.rsl_rl_ppo_cfg import LOW_FREQ_SCALE
from engineai_rl_lab.tasks.tracking.tracking_env_cfg import TrackingEnvCfg


@configclass
class T800proFlatEnvCfg(TrackingEnvCfg):
    def __post_init__(self):
        super().__post_init__()

        self.scene.robot = T800pro_CYLINDER_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        self.scene.env_spacing = 3.5
        self.actions.joint_pos.scale = T800pro_ACTION_SCALE
        ...
        self.rewards.undesired_contacts.params["sensor_cfg"].body_names = [
            r"^(?!LINK_ANKLE_ROLL_L$)(?!LINK_ANKLE_ROLL_R$)(?!LINK_WRIST_ROLL_L$)(?!LINK_WRIST_ROLL_R$).+$"
        ]
...
@configclass
class T800proFlatWoStateEstimationEnvCfg(T800proFlatEnvCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class T800proFlatLowFreqEnvCfg(T800proFlatEnvCfg):
    def __post_init__(self):
        super().__post_init__()
